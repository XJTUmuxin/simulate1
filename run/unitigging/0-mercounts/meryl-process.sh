...
if [ -e ./test.ms22.histogram -a -e ./test.ms22.dump -a -e ./test.ms22.ignore.gz ] ; then
  exit 0
fi
...
if [ -e test.ms22.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e test.ms22.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e test.ms22.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'test.ms22.histogram', 'test.ms22.dump' and 'test.ms22.ignore.gz' exist in 'unitigging/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./test.ms22/merylIndex ] ; then
  /home/zmx/work/canu/build/bin/meryl threads=1 memory=2 \
    greater-than 1 \
      output test.ms22.WORKING \
      union-sum  \
        ./test.01.meryl \
  && \
  mv -f ./test.ms22.WORKING ./test.ms22
...
if [ ! -e ./test.ms22.histogram ] ; then
  /home/zmx/work/canu/build/bin/meryl threads=1 memory=1 \
    statistics ./test.ms22 \
  > ./test.ms22.histogram.WORKING \
  && \
  mv ./test.ms22.histogram.WORKING ./test.ms22.histogram
fi
...
if [ ! -e ./test.ms22.dump ] ; then
  /home/zmx/work/canu/build/bin/meryl threads=1 memory=2 \
    print ./test.ms22.##.dump \
      at-least distinct=0.9999 \
        ./test.ms22

  cat ./test.ms22.??.dump > ./test.ms22.dump
  rm -f ./test.ms22.??.dump
fi
...
if [ ! -e ./test.ms22.ignore.gz ] ; then
  numKmers=`wc -l < ./test.ms22.dump`
  totKmers=`/home/zmx/work/canu/build/bin/meryl statistics ./test.ms22 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./test.ms22.dump | gzip -1c > ./test.ms22.ignore.gz
fi
