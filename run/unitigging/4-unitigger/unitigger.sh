...
if [ -e test.ctgStore/seqDB.v001.tig ]; then
  exists=true
else
  exists=false
fi
...
if [ -e ./test.ctgStore ] ; then
  mv ./test.ctgStore ../test.ctgStore
fi
...
#
if [ -e ../test.ctgStore -a \
   ! -e ../test.ctgStore/seqDB.v001.sizes.txt ] ; then
  $bin/tgStoreDump \
    -S ../../test.seqStore \
    -T ../test.ctgStore 1 \
    -sizes -s 10000 \
   > ../test.ctgStore/seqDB.v001.sizes.txt
fi
